...
Factory 자동화는 실행 요소, 인증정보, 의존성 해석과 도구 체계를 다음 원칙으로 제한한다.

- 외부 실행 요소는 검증한 전체 커밋 해시로 고정하고 같은 줄에 대응 공개 버전을 기록한다
- 검증 작업은 저장소 읽기 권한만 가지며 checkout 이후 인증 Git 작업이 없으므로 인증정보를 유지하지 않는다
- `pubspec.lock`의 해석과 hosted package 콘텐츠 해시가 다르면 `dart pub get --enforce-lockfile`에서 중단한다
- 실제 통합 자동화의 Flutter 실행 요소는 전체 해시로 고정하되 내부의 이동 가능한 캐시 실행 요소를 호출하지 않도록 캐시를 비활성화한다
- 캐시 비활성화로 원격 실행 제한을 충족하지 못하면 제한 시간을 임의로 늘리거나 캐시를 다시 켜지 않고 별도 실행 프로필 승인을 요구한다
- `stable` SDK 채널은 최신 안정판 호환성을 확인하기 위한 의도적인 가변 입력이며 실행마다 실제 도구 버전을 Evidence로 남긴다. 완전한 재현성을 의미하지 않으며 Release 기준점에는 별도 고정 Evidence가 필요하다
