...
- GitHub 자동화의 저장소 권한은 `contents: read`로 유지하고 외부 실행 요소를 검증된 전체 커밋 해시로 고정했다
- checkout 인증정보를 유지하지 않으며 잠금 파일의 해석과 hosted package 콘텐츠 해시가 다르면 의존성 준비 단계에서 중단한다
- 고정된 Flutter 실행 요소 안의 이동 가능한 `actions/cache@v5`가 실행되지 않도록 캐시를 비활성화했다
- 직접 의존성과 잠금 파일에는 필수 변경이 없으며 최신판 차이가 있는 간접 의존성 두 개에서 보안 권고, 폐기 또는 회수 상태가 확인되지 않았다
- `stable` SDK 채널은 최신 안정판 호환성 신호로 유지하며 캐시 없는 실제 원격 통합 실행 시간은 후속 Evidence로 남긴다
- 저장소별 Dependabot 알림은 인증된 조회를 수행하지 못해 남은 수동 확인으로 유지한다
- P6-2 완료에는 로컬 검증, 독립 QA, 사용자 결과 승인과 별도 승인된 push 이후 원격 자동화 성공 Evidence가 필요하다
...
현재 진행 중인 작업은 **P6-2 — 의존성과 자동화 공급망 검토** 하나뿐이다. 구현 후보와 로컬 검증이 존재하며 독립 품질 보증 수정 재확인, 사용자 결과 승인과 별도 승인된 푸시 후 원격 자동화 증거가 남았다.
